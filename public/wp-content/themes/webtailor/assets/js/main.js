/* 
Functions to call on every page 
*/
// 1.1 AOS animations
...
(function($){ $(document).ready(function() {
    if($('.menu-item').length > 0 ){
        ...
        $('.menu-item a').click(function(e) {
            ...
            $('html, body').animate({
                scrollTop: $(targetHref).offset().top // Add it to the calculation here
            }, 800);
...
// 1.5 give class active to links
(function($){ $(document).ready(function() {
    $("[href]").each(function() {
...
// 1.6 popup button
(function($){ $(document).ready(function() {
    $('.pop-up .closepop').click(function() {
        $(this).closest('.pop-up-container').removeClass( "show" );
    });
...
// 1.7 dropdown menu animation
(function($){ $(document).ready(function() {
    $('.header-menu.desktop li.menu-item-has-children').hover(function() {
...
(function($){
    $(window).scroll(function() {
        ...
        if (scroll >= 1) {
            $(".nav-menu-wrapper").addClass("sticky-nav");
        } else {
            $(".nav-menu-wrapper").removeClass("sticky-nav");
        }
...
// video modal
(function($){ $(document).ready(function() {
    if($('#all-video-results').length > 0 ){
        $(document).on('click','.video-button', function(event) {
            event.preventDefault();
            let video = $(this).attr("href");
            let embed = video.replace("www.youtube.com/watch?v=", "www.youtube.com/embed/");
            $("#youtube-iframe").attr("src", embed);
        });
        $('#video-container .closepop').click(function() {
...
function filterVideos() {
    jQuery.ajax({
        ...
        url: "",
        data: jQuery('#search-filter-form-182').serialize(),
...
(function($) {
    let timer;
    let delay = 1000;
    $(document).on('click', '#_sft_reset a', function (event) {
        $('#_sf_search').val('');
        $('#_sft_periode').val('');
        $('#_sft_thema').val('');
        event.preventDefault();
    ...
    $(document).on('input', '#_sf_search', function() {
        window.clearTimeout(timer);
        timer = window.setTimeout(function(){
            filterVideos();
        }, delay);
    });
    $(document).on('change','#_sft_periode',function(){
        filterVideos();
    });
    $(document).on('change','#_sft_thema',function(){
        filterVideos();
    });
})(jQuery);
// scrolling and getting
(function($) {
    let isGetting = false;
    $(window).scroll(function() {
        let offset = $(document.getElementById("footer")).offset().top;
        let empty = $('#all-video-results').html().includes('Sorry, er zijn geen video\'s gevonden');
        if (isGetting === false && empty === false && window.scrollY > offset - 1000) {
            let numberOfVideos = $(document.getElementsByClassName("video")).length;
            isGetting = true;
            $.ajax({
                type: "GET",
                contentType: "application/json; charset=utf-8",
                url: "",
                data: $('#search-filter-form-182').serialize() + "&scroll-videos-ajax=" + numberOfVideos,
                success: function (result) {
                    let videosDiv = $('#all-video-results');
                    let videos = videosDiv.html();
                    videosDiv.html(videos + result);
                    isGetting = false;
                }
            });
        }
